      ...
      <el-col :span="24">
        ...
        <el-table :data="data" :border="true" :height="800" :show-header="true" :highlight-current-row="true">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="primary" @click="editArticle(scope.row.id)">编辑</el-button>
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        //获取数据
        getData(page) {
          this.$http.$request('/admin/articles','get',{pageNum: page, isDelete: 0}).then(response => {
            const data = response.data.data
            if(data) {
              this.data = data.list
              this.pageInfo = data
